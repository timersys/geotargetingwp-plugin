...
class GeotWP_RestApi {
	...
	public function __construct() {
		//add_filter( 'rest_url_prefix', [ $this, 'prefix_api_rest' ] );
		add_action( 'rest_api_init', [ $this, 'register_api_init'] );
	}

	
	//public function prefix_api_rest() {
	//	return apply_filters( 'GeotWP/apirest/prefix', 'geot' );
	//}

	protected function get_api_base() {
		return apply_filters( 'GeotWP/apirest/namespace', 'geot/v1' );
	}

	//https://timersys.local/wp-json/geot/v1/shortcode/34
	//'/shortcode/(?P<id>\d+)',
	 ...
	 * @return string
	 */
	public function print_api_rest( WP_REST_Request $request ) {

		/*update_option('pclon_1', print_r($_REQUEST,true));
		update_option('pclon_2', print_r($_POST,true));
		update_option('pclon_3', print_r($request->get_body_params(),true));
		update_option('pclon_4', print_r($request->get_params(),true));
		update_option('pclon_5', print_r($request->get_body(),true));*/
	...
	private function geo_redirects() {
		...
		return $GeoRedirect->handle_ajax_redirects( $this->data );
	}
	...
	private function geo_blockers() {
		...
		return $GeoBlocker->handle_ajax_blockers( $this->data );
	}
